//
//  Constants.m
//  CopyMate
//
//  Created by hewig on 12/7/13.
//  Copyright (c) 2013 kernelpanic.im. All rights reserved.
//

#import "Constants.h"

NSString * const MASPrefKeyCopyShortcut          = @"CMCopyShortcut";
NSString * const MASPrefKeyDefaultFormatShortcut = @"CMDeFormatShortcut";
NSString * const MASPrefKeyAlterFormatShortcut   = @"CMAlterFormatShortcut";
NSString * const CopyMateDefaultFormat           = @"CMDefaultFormat";
NSString * const CopyMateAlterFormat             = @"CMAlterFormat";
NSString * const CopyMateStartAtLogin            = @"CMStartAtLogin";
NSString * const CopyMateHomeLink                = @"https://github.com/hewigovens/copymate";
NSString * const CopyMateHome                    = @"github/copymate";
NSString * const CopyMateNotFirstRun             = @"CopyMateNotFirstRun";